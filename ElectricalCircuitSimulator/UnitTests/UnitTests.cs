...
namespace UnitTests
{
    ...
    public class TestCircuitSimulationBlocks
    {
        ...
        public void TestCircuitComponent()
        {
            ...
            AssertAction.VerifyAssert(() => oCircuitComponent = new LinearCircuitComponent(5, 5, false), "Expected 'Node values must not be the same!' error, did not get it!");
            AssertAction.VerifyAssert(() => oCircuitComponent = new LinearCircuitComponent(0, 0, false), "Expected 'Node values must be greater than or equal to 0!' error, did not get it!");

            oCircuitComponent = new LinearCircuitComponent(0, 5, false);
            Assert.IsTrue(oCircuitComponent.hasGround() == false, "Incorrect value! Expected false");
            ...
            oCircuitComponent = new LinearCircuitComponent(2, 7, true); // Check for memory access problems
        }
